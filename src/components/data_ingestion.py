...
class DataIngestion:
    ...
    def initiate_data_ingestion(self):
        ...
        try:
            df=pd.read_csv('notebook/data/stud.csv') #be creative to import of any database
            logging.info('Read the dataset as dataframe')
            
            os.makedirs(os.path.dirname(self.ingestion_config.train_data_path),exist_ok=True) #only creates (if don't exists) the folder 'artifacats'
            
            df.to_csv(self.ingestion_config.raw_data_path,index=False,header=True)#save the hole dataset
            
            logging.info("Train test split initiated")
            train_set,test_set=train_test_split(df,test_size=0.2,random_state=42)
            
            train_set.to_csv(self.ingestion_config.train_data_path,index=False,header=True) #creates the file train.csv inside the foles artifacts
